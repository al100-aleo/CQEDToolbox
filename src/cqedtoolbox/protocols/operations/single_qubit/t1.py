...
class IncreaseDelayCorrection(Correction):
    ...
    def reset(self) -> None:
        if self._original_delay is not None:
            self.delay_param(self._original_delay)
        self._count = 0


class IncreaseAveragingCorrection(Correction):
    name = "increase_averaging"
    description = "Increase number of repetitions and reset delay"
    triggered_by = "quality_check"

    def __init__(self, reps_param, delay_correction: IncreaseDelayCorrection,
...
class T1Operation(ProtocolOperation):
    ...
    def correct(self, result: EvaluateResult) -> EvaluateResult:
        ...
        self.report_output.append(
            f"## T1 Measurement\n"
            f"Measured T1 relaxation time with SNR threshold: {snr_min:.1f}\n"
